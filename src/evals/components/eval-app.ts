...
export class EvalApp extends Container {
  ...
  private renderState() {
    ...
    if (this.state.status === 'loading') {
      this.addChild(new Text(theme.bold(theme.primary('Cramer-Short Eval')), 0, 0));
  ...
  private renderRunningState() {
    ...
    this.addChild(new Text(`${theme.bold(theme.primary('Cramer-Short Eval'))}${theme.muted(datasetLabel)}`, 0, 0));
