 ...
 *   - daysToConverge ≤ 7  ⇒ up to +15% boost (linearly, max at 1d)
 *   - daysToConverge ≥ 30 ⇒ down to −10% damp (saturates at 30d+)
 *   - Linear interpolation in (7, 30)
...
export function convergenceTimeFactor(r: ConvergenceResult): number {
  ...
  if (days <= FAST_THRESHOLD_DAYS) return 1 + FAST_BOOST;
  if (days >= SLOW_THRESHOLD_DAYS) return 1 - SLOW_DAMP;
  // Linear interp between (7, +15%) and (30, −10%).
