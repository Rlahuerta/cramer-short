...
describe('Walk-forward HYPE short-horizon benchmark', () => {
  it('keeps the HYPE mutator contract centered on 1d/2d/3d while treating 7d/14d as guardrails', () => {
    const baseline = {
      ...
      h2: { directionalAccuracy: 0.49 },
      h3: { directionalAccuracy: 0.39 },
      h7: { directionalAccuracy: 0.29 },
      h14: { directionalAccuracy: 0.26 },
    };
    ...
    const regressedPrimaryHorizons = {
      ...
      h1: { directionalAccuracy: 0.55 },
      h2: { directionalAccuracy: 0.45 },
      h3: { directionalAccuracy: 0.34 },
    };
  ...
  integrationIt('emits machine-readable HYPE short-horizon metrics across 1d/2d/3d/7d/14d', async () => {
    const metricsByHorizon: Record<string, Omit<HorizonMetrics, 'steps' | 'errors'>> = {};
    const lines: string[] = ['', '═══ HYPE SHORT-HORIZON BENCHMARK ═══'];
