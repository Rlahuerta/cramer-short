/**
 * Phase 8: Warmup-window comparison harness.
 *
 * Compares the Phase 4 control (warmup=120, trendPenaltyOnlyBreakConfidence=true)
 * against warmup-window candidates (warmup=90 and warmup=60, both with
 * trendPenaltyOnlyBreakConfidence=true) on the 6-ticker 7d/14d/30d fixture
 * universe.
 *
 * Warmup is horizon-sensitive: reducing it changes how many historical
 * observations feed the Markov transition matrix, which affects regime
 * detection, break detection, and calibration quality at different horizons.
 * We therefore report per-horizon metrics and enforce per-horizon guardrails.
 *
 * Evidence basis (from Phase 1 ablation):
 *   - warmup=90 improved aggregates most at 14d
 *   - warmup=60 improved aggregates most at 30d
 *   - Phase 4 (warmup=120 + trendPenaltyOnly) remains the best overall control
 *   - postBreakShortWindow=60 is broadly harmful (not revisited here)
 *   - Phase 7 was a negative result (not revisited here)
 *
 * Run: bun run src/tools/finance/backtest/phase8-warmup-window-comparison.ts
 */
...
export interface Phase8Artifact {
  generatedAt: string;
...
// Guardrails: thresholds tied to Phase 4 baseline behavior.
// Warmup reduction always sacrifices some data; these ensure we don't regress.
...
// Per-horizon guardrails: warmup changes are horizon-sensitive.
const HORIZON_BREAK_CONTEXT_MAX_LOSS = 0.015;
// For the warmup=90 candidate: we expect improvement at 14d but not at 7d or 30d.
const HORIZON_14D_BREAK_CONTEXT_MIN_GAIN_W90 = 0.005;
// For the warmup=60 candidate: we expect improvement at 30d but not at 7d or 14d.
const HORIZON_30D_BREAK_CONTEXT_MIN_GAIN_W60 = 0.005;

const CANDIDATES: WarmupCandidate[] = [
  { id: 'warmup-90', warmup: 90 },
  { id: 'warmup-60', warmup: 60 },
];
...
export function evaluatePhase8Thresholds(params: {
  candidateId: string;
  warmup: number;
  ...
  const { candidateId, warmup, candidate, baselinePerHorizon } = params;
  ...
  // Warmup-candidate-specific evidence-based expectations
  if (warmup === 90) {
    const h14 = candidate.perHorizonDelta.find(entry => entry.horizon === 14);
    if (h14 && h14.breakContextDirectionalAccuracy < HORIZON_14D_BREAK_CONTEXT_MIN_GAIN_W90) {
      failureReasons.push(
        `warmup=90 expected 14d break-context gain ≥ ${formatDelta(HORIZON_14D_BREAK_CONTEXT_MIN_GAIN_W90)}, got ${formatDelta(h14.breakContextDirectionalAccuracy)}`,
      );
    }
  }

  if (warmup === 60) {
    const h30 = candidate.perHorizonDelta.find(entry => entry.horizon === 30);
    if (h30 && h30.breakContextDirectionalAccuracy < HORIZON_30D_BREAK_CONTEXT_MIN_GAIN_W60) {
      failureReasons.push(
        `warmup=60 expected 30d break-context gain ≥ ${formatDelta(HORIZON_30D_BREAK_CONTEXT_MIN_GAIN_W60)}, got ${formatDelta(h30.breakContextDirectionalAccuracy)}`,
      );
    }
  }
...
export function selectWinner(
  candidates: CandidateResult[],
): { candidateId: string | null; reason?: string } {
  const passing = candidates.filter(c => c.passesThresholds);
  if (passing.length === 0) {
    return {
      candidateId: null,
      reason: 'No candidate passed all Phase 8 thresholds against the Phase 4 baseline',
    };
  }

  if (passing.length === 1) {
    return { candidateId: passing[0].candidateId };
  }

  // Multiple passing candidates: rank by weighted per-horizon break-context improvement.
  // warmup=90's expected strength is 14d; warmup=60's is 30d.
  // Weight each horizon's break-context DA delta equally.
  const scored = passing.map(c => {
    const improvement = c.perHorizonDelta.reduce(
      (sum, entry) => sum + entry.breakContextDirectionalAccuracy,
      0,
    );
    return { candidate: c, score: improvement };
  });

  scored.sort((a, b) => b.score - a.score);
  return { candidateId: scored[0].candidate.candidateId };
}
...
export async function runComparison(
  ...
  for (const candidate of CANDIDATES) {
    ...
    const thresholdEvaluation = evaluatePhase8Thresholds({
  ...
  const winner = selectWinner(candidateResults);
...
export function printSummary(artifact: Phase8Artifact): void {
  console.log('Phase 8: Warmup-Window Comparison');
  console.log('═══════════════════════════════════════════════════════════════');
  console.log(
  ...
  for (const c of artifact.candidates) {
    ...
    console.log(
      `  ${c.candidateId.padEnd(16)} | ${horizonParts.join(' | ')}`,
    );
  }
  ...
  for (const c of artifact.candidates) {
    console.log(
      `  ${c.candidateId.padEnd(16)} | Δrc020=${formatDelta(c.deltaVsBaseline.breakContextRC020Accuracy)}/${formatDelta(c.deltaVsBaseline.breakContextRC020Coverage)} | Δrc030=${formatDelta(c.deltaVsBaseline.breakContextRC030Accuracy)}/${formatDelta(c.deltaVsBaseline.breakContextRC030Coverage)}`,
    );
  }
  ...
  if (artifact.winner.candidateId) {
    ...
    console.log('Result: No candidate passed all Phase 8 thresholds.');
    if (artifact.winner.reason) {
      console.log(`Reason: ${artifact.winner.reason}`);
    }
  }
}
...
export async function main(): Promise<void> {
  ...
  const artifactPath = join(artifactDir, 'phase8-warmup-window-comparison.json');
