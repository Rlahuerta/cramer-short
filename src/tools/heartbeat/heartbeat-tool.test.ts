...
// don't touch the real .dexter/HEARTBEAT.md during the test run.
// The heartbeat-tool uses dexterPath() which joins getDexterDir() + filename.
// We override getDexterDir's underlying folder by writing to the real path but
...
describe('heartbeatTool — view', () => {
  ...
  it('returns the file content when checklist exists', async () => {
    const dexterDir = join(tmpDir, '.dexter');
...
describe('heartbeatTool — update', () => {
  ...
  it('writes content to HEARTBEAT.md and returns summary with item count', async () => {
    ...
    const file = join(tmpDir, '.dexter', 'HEARTBEAT.md');
    expect(existsSync(file)).toBe(true);
  });

  it('creates the .dexter directory if it does not exist', async () => {
    const dexterDir = join(tmpDir, '.dexter');
  ...
  it('creates a gateway.json enabling heartbeat when items are present', async () => {
    ...
    const gatewayPath = join(tmpDir, '.dexter', 'gateway.json');
